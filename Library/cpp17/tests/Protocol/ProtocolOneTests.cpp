...
public:
    ...
    {
        auto memoryStream = std::make_shared<Rpc2::Streams::MemoryStream>(SmallCacheSize);

        {
            auto bufferedOutputStream = std::make_shared<Rpc2::Streams::BufferedOutputStream>(memoryStream, SmallCacheSize);
    ...
    void TestPrimitiveParsing(const std::string& encodedValue, const TValue& expectedValue) const
    {
        ...
        auto bufferedInputStream = std::make_shared<Rpc2::Streams::BufferedInputStream>(memoryStream, SmallCacheSize);
...
TEST_F(ProtocolOneTestFixture, PeekNextObjectType_FunctionCallInStream_CorrectTypeReturned)
{
    ...
    auto bufferedInputStream = std::make_shared<Rpc2::Streams::BufferedInputStream>(memoryStream);
...
{
    const auto expectedObjectType = Rpc2::Protocol::ObjectType::FunctionCall;
    ...
    const auto memoryStream = std::make_shared<Rpc2::Streams::MemoryStream>();
    const auto bufferedInputStream = std::make_shared<Rpc2::Streams::BufferedInputStream>(memoryStream);
...
{
    const auto expectedObjectType = Rpc2::Protocol::ObjectType::Event;
...
{
    const auto objectType = Rpc2::Protocol::ObjectType::FunctionCall;
...
{
    const auto objectType = Rpc2::Protocol::ObjectType::Event;
    const std::string identifier("Identifier");
    ...
    auto memoryStream = std::make_shared<Rpc2::Streams::MemoryStream>(SmallCacheSize);
    {
        ...
        auto object = protocol->WriteObject(bufferedOutputStream, objectType, identifier, Rpc2::Protocol::ObjectToken(""));
        object.reset();
    }
...
TEST_F(ProtocolOneTestFixture, ReadValue_String_CorrectlyParsed)
{
...

    {
        ...
        auto protocol = std::make_shared<Rpc2::Protocol::One::ObjectReader>(bufferedInputStream);
...
TEST_F(ProtocolOneTestFixture, WriteValue_String_CorrectlySerialized)
{
    ...
    {
        auto bufferedOutputStream = std::make_shared<Rpc2::Streams::BufferedOutputStream>(memoryStream, SmallCacheSize);
        auto objectWriter = std::make_shared<Rpc2::Protocol::One::ObjectWriter>(bufferedOutputStream);
...
TEST_F(ProtocolOneTestFixture, WriteList_IntList_AllValuesWritten)
{
    ...
    {
        auto bufferedOutputStream = std::make_shared<Rpc2::Streams::BufferedOutputStream>(memoryStream, SmallCacheSize);
        auto protocol = std::make_shared<Rpc2::Protocol::One::ObjectWriter>(bufferedOutputStream);
...
TEST_F(ProtocolOneTestFixture, ReadObject_NestedObjects_ObjectsParsed)
{
    ...
    memoryStream->Write(input.data(), input.size());
    auto bufferedInputStream = std::make_shared<Rpc2::Streams::BufferedInputStream>(memoryStream, SmallCacheSize);
    auto objectReader = std::make_shared<Rpc2::Protocol::One::ObjectReader>(bufferedInputStream);
