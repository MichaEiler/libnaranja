...
TEST_F(MemoryStreamTestFixture, Write_StreamClosed_ThrowsException)
{
    auto memoryStream = std::make_shared<Rpc2::Streams::MemoryStream>();
    memoryStream->Close();
    ASSERT_THROW(memoryStream->Write(nullptr, 0), Rpc2::Exceptions::StreamClosed);
}
...
TEST_F(MemoryStreamTestFixture, Write_LargerThanCache_DataProcessed)
{
    ...
    auto memoryStream = std::make_shared<Rpc2::Streams::MemoryStream>(cacheSize);
    ...
    std::thread consumerThread([&](){
        ...
        catch(const Rpc2::Exceptions::StreamClosed&)
        { } 
    });
