...
class CppGenerator:
    def __init__(self, documents: typing.List[RpcDocument], outputDirectory: str, projectName: str):
        ...
        self._templatePaths = os.path.dirname(os.path.realpath(__file__)) + "/Templates/cpp11/"
    ...
    def _renderServices(self):
        ...
        for document in self._documents:
            if document.hasServices():
                template = Template(self._readTemplate("YourServiceInterfaces.hpp.jinja2"))
                template.globals["include"] = includer
                registerTemplateFunctions(template)

                result = template.render(projectName = self._projectName,
                                         generatorVersion = "0.0.0.1",
                                         generatorName = "RpcTool",
                                         generationDate = str(datetime.datetime.today()),
                                         document = document)
                self._writeResult("{0}ServiceInterfaces.hpp".format(document.name), result)

            if document.hasTypes():
                template = Template(self._readTemplate("YourTypes.hpp.jinja2"))
                template.globals["include"] = includer
                registerTemplateFunctions(template)
                ...
                result = template.render(projectName = self._projectName,
                                         ...
                                         generationDate = str(datetime.datetime.today()),
                                         document = document)
                self._writeResult("{0}Types.hpp".format(document.name), result)
